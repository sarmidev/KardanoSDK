package scratch.signingbackend

import kotlin.test.Test
import kotlin.test.assertContentEquals
import kotlin.test.assertFalse
import kotlin.test.assertTrue
// `sign`, `verify`, and `deriveXpub` are generated by UniFFI into this same
// `scratch.signingbackend` package (see build.gradle.kts uniffi { generateFromLibrary { packageName = ... } }),
// so no explicit import is needed.

/**
 * Block 1.10b provisioning spike — JVM KAT (ADR-0016 §7c, §7e step 1).
 *
 * Reproduces the ADR-0016 §3 primary vector against the real JVM/JNA-backed
 * UniFFI wrapper (not the plain Rust `cargo test`, which already passed
 * separately). This is the first of the spike's required target-coverage
 * checks: JVM KAT reproduced through the actual generated Kotlin bindings.
 *
 * Vector source: `ed25519-bip32` crate 0.4.2, `src/tests.rs`, `xprv_sign` /
 * `verify_signature` (`D1_H0`), cited by CIP-3. Copied verbatim from
 * ADR-0016 §3, not invented.
 */
