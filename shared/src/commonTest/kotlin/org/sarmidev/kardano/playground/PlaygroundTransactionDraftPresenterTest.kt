...
class PlaygroundTransactionDraftPresenterTest {
    ...
    fun okDraft_producesSuccessRowsWithCountsFeeAndBodyPreview() {
        ...
        assertEquals(LovelaceDisplay.ada(draft.outputs[0].amount), rowByLabel["Payment"])
        assertEquals(LovelaceDisplay.ada(draft.fee), rowByLabel["Network cost"])
    }
    ...
    fun okDraft_withChange_includesChangeRow() {
        ...
        assertEquals(LovelaceDisplay.ada(draft.outputs[1].amount), rowByLabel["Change back"])
    }
