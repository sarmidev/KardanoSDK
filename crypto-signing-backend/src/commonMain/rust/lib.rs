//! `:crypto-signing-backend` — project-owned Cardano extended Ed25519-BIP32 signing backend
//! (ADR-0015 Block 1.10; adopted per ADR-0016 §9, Option R1).
//!
//! Exposes Cardano extended Ed25519-BIP32 `sign`/`verify` through UniFFI by delegating directly
//! to `ed25519_bip32::XPrv::sign` / `XPub::verify` — the reference primitive cited by ADR-0016 §3
//! and CIP-3. This crate performs no cryptographic computation of its own: it only validates input
//! byte lengths (which the underlying crate already rejects) and moves bytes across the FFI
//! boundary. There is no handwritten cryptography here and no plain RFC-8032 seed-based Ed25519.
//!
//! This is the permanent, vendored replacement for the disposable `scratch-signing-backend`
//! provisioning spike (ADR-0016 §8). The generated Kotlin bindings and the per-target native
//! artifacts built from this crate are committed under
//! `src/{commonMain,jvmMain,androidMain}/kotlin`, `src/jvmMain/resources`,
//! `src/androidMain/jniLibs`, and `src/nativeInterop`; the exact offline build commands and pinned
//! toolchain versions are recorded in `README.md`.
...
fn sign(xprv: Vec<u8>, message: Vec<u8>) -> Result<Vec<u8>, SigningBackendError> {
    let xprv = XPrv::from_slice_verified(&xprv)
        .map_err(|_| SigningBackendError::InvalidExtendedPrivateKey)?;
...
mod tests {
    ...
    /// ADR-0016 §3 primary KAT: `ed25519-bip32` 0.4.2 `src/tests.rs` `xprv_sign` /
    /// `verify_signature`, the `D1_H0` fixture. Copied verbatim from the cited source, not
    /// invented.
    #[test]
    fn reproduces_adr_0016_primary_kat() {
        ...
        let chain_code =
            from_hex("608763770eddf77248ab652984b21b849760d1da74a6f5bd633ce41adceef07a");
        ...
        // The vector's own verify test derives the public key via `XPrv::public()` rather than
        // hardcoding it; do the same here.
