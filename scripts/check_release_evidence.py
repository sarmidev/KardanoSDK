...
8. docs/evidence/scope_binding.json's two-commit seal is independently
   ...
   immediate parent, evidence_commit is literal HEAD's exact immediate
   parent -- HEAD must BE the seal commit, unconditionally, with no
   merge-ref or other exception of any kind -- and every sealed evidence
   ...
   the seal still fails, and so does ANY two-(or more-)parent merge
   commit at HEAD, including a genuine GitHub `pull_request` merge-ref:
   this checker never inspects `GITHUB_EVENT_NAME`/`GITHUB_EVENT_PATH` at
   all. A caller that needs this checker to validate something other
   than literal HEAD (e.g. a future `push` to `main` whose HEAD is a
   merge commit) must `git checkout --detach` the exact commit to check
   BEFORE invoking this script -- see `scripts/select_seal_checkout_head.py`
   and its own module docstring for the one place that kind of event-bound
   selection is allowed to happen, kept entirely separate from this
   checker's own strict, event-independent seal verification.
...
def check_scope_binding_seal() -> list[str]:
    ...
    - `evidence_commit` is EXACTLY literal HEAD's immediate parent (not
      ...
      seal without invalidating it). This is unconditional: there is no
      merge-ref, event-bound, or any other exception here at all -- a
      2026-08-25 independent review found the previous GitHub
      `pull_request` merge-ref exception was a checker-level special
      case that could not be justified once the workflow itself checks
      out the exact PR head SHA for `pull_request` (see
      `.github/workflows/verify.yml`'s `legal-evidence-scan` job); this
      checker now NEVER inspects `GITHUB_EVENT_NAME`/`GITHUB_EVENT_PATH`
      and always requires literal HEAD's first parent to be
      `evidence_commit`. A caller that legitimately needs to validate a
      commit other than literal HEAD (a future `push` to `main` whose
      HEAD is a genuine merge commit) must `git checkout --detach` the
      exact commit to check BEFORE invoking this script -- see
      `scripts/select_seal_checkout_head.py`, which performs that
      event-bound selection entirely outside this module and hands this
      checker a literal, already-selected HEAD to check strictly. This
      function does NOT independently re-verify full source-scope
      completeness beyond that immediate-parent check -- see
      `check_full_source_scope_seal()` below for the broader,
      import-graph-independent completeness check this two-commit design
      relies on as defense-in-depth.
