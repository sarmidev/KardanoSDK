...
| 7 | `fix/native-build-and-platform-evidence` | Gate 2 Linux | Linux x86-64 JVM candidate rebuild on pinned `ubuntu-22.04` (glibc 2.35). Fresh Phase B at `b59009d` run `32676885035` succeeded (A/B + compare + JVM KAT). Artifacts expire 2026-09-07. Not promoted. Independent A/B re-review still required. Windows not started. |
...
- **Native rebuild evidence on `fix/native-build-and-platform-evidence`.**
  ...
  Versym 0, canonical dynsym 0, globally unique `vna_other`, and
  fail-closed `readelf --dyn-syms --wide` corroboration. Fresh Phase B
  at `b59009d` is run `32676885035` (success; artifacts expire
  2026-09-07; not downloaded). Verify `32676885074` and native
  `32676885086` were green at that tip. Runs `32675120131` and
  `32675524925` are superseded.
  CHECKSUMS still has
...
re-review is GO). Fresh Phase B CI at `b59009d` is run `32676885035`
(success, not promoted). Independent re-review is still required.
Do not promote. Do not start Windows until then. Residual owner work:
