...
| W3-3 / lock + verification | **Further remediated** on `fix/build-and-ci-reproducibility` commit 4 — STRICT per-project Gradle lockfiles, SHA-256 `gradle/verification-metadata.xml`, `rust-toolchain.toml` 1.97.0, Cargo exact pins and `--locked`. Lint CI remains the later lint commit. |
