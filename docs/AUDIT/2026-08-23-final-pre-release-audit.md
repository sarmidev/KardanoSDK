...
| NF-5 | **Further remediated** — match-by-match scanner with `path:line:column`, longest phrase first, sentence-bounded negation, Markdown-emphasis normalization, and one reviewed compound (`display-safe`). Exclusions are exact files, not `docs/DECISIONS/` or `docs/AUDIT/` prefixes. Tracked files come from `git ls-files -z` and include `.swift` / `.xml` / `.properties` / `.toml` / `.yaml` / `.yml` / `.json`. CI runs the unit tests before the scan. |
