...
| NF-5 | **Further remediated** — match-by-match scanner with `path:line:column`, longest phrase first, sentence-bounded negation, and Markdown-emphasis normalization. Hyphen compounds are not exempt. Evolving ADRs and the append-only Phase 1 log are scanned; unavoidable historical wording is allowlisted per occurrence (path + line SHA-256 + phrase + occurrence index). Whole-file exclusions remain only for immutable archived snapshots and circular policy/test data. Suffix matching is case-insensitive. Tracked files come from `git ls-files -z`. CI runs the unit tests before the scan. |
