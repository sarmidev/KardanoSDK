...
| W3-3 / toolchain freshness | **In progress** on `fix/build-and-ci-reproducibility` commit 2 — Gradle 9.7.1, AGP 9.3.1, Kotlin 2.4.10, target SDK 37. Lint CI is the later lint commit. |
...
| W3-3 / lock + verification | **Further remediated** on `fix/build-and-ci-reproducibility` commit 4 — STRICT per-project Gradle lockfiles, SHA-256 `gradle/verification-metadata.xml`, `rust-toolchain.toml` 1.97.0, Cargo exact pins and `--locked`. |
| W3-3 / lint CI | **Further remediated** on the same branch commit 5 — Debug and Release lint run in `verify.yml` with `warningsAsErrors`. Freshness detectors disabled because versions are locked and checksum-verified. Monochrome adaptive layer, nodpi splash assets, 1px inset on filled-square legacy launchers. |
