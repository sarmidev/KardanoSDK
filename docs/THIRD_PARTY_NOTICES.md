...
| Gradle lockfiles and `gradle/verification-metadata.xml` | Build-input metadata (not redistributed) | Per-project locked coordinates and SHA-256 artifact checksums generated by Gradle 9.7.1. Review notes are in [DEPENDENCY_REVIEW.md](DEPENDENCY_REVIEW.md). | Not a third-party library; checksums describe Maven/Google artifacts already listed above |
