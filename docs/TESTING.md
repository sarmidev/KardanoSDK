...
compiled in that job. It writes `security-events` only. A default-branch
CodeQL analysis already exists. Do not claim the Intersect attestation
code-scanning row is GREEN until an official generated report says so.

`.github/workflows/intersect-attestation.yml` is `workflow_dispatch`
only on `ubuntu-24.04`. There is no automatic pull-request or push
trigger and therefore no recurring PR cost or third-party script run on
ordinary review. Workflow and job permissions are exactly
`contents: read` plus `security-events: read`. GitHub's Actions
permissions schema documents `security-events: read` as the key that
lists code-scanning alerts; the official script calls
`GET /repos/{owner}/{repo}/code-scanning/alerts`. There is no narrower
Actions permission and no write scope. No PAT or repository secret is
added. Default-branch CodeQL analysis already exists (owner-token
alerts API HTTP 200, zero alerts). Official run
https://github.com/sarmidev/KardanoSDK/actions/runs/35452860848 is a
valid HTML/PDF/Markdown artifact whose Overall was AMBER solely because
the previous `GITHUB_TOKEN` had only `contents: read` and could not see
that analysis. This permission is read-only attestation visibility, not
a new scanner. Early xvfb/PDF diagnostics
(https://github.com/sarmidev/KardanoSDK/actions/runs/35451128768 and
https://github.com/sarmidev/KardanoSDK/actions/runs/35451205414) remain
historical evidence only. The Phase 1 submission PDF must be generated
by a deliberate owner dispatch after merge: `gh workflow run
intersect-attestation.yml --ref main`. A branch dispatch can check
whether the token can list alerts while the script still reads default
`main`; do not overwrite the downloaded official PDF and do not treat
that branch artifact as the submission. Reviewer/PR CI cannot validate
`workflow_dispatch` behavior; a post-merge owner run is required. The
job clones `IntersectMBO/Project-Compliance-Attestation` at
...
official script queries the default branch, so a pre-merge run is
diagnostic of `main` only. apt PDF tools are runner-image packages; PDF
bytes are not claimed deterministic. Do not claim GREEN unless an
official generated report says GREEN. See
`scripts/intersect_attestation_pin.py` and
