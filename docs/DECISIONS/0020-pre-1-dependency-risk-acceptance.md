...
| `uniffi` (crate) | `"0.29.4"` (lock 0.29.5) | 0.32.0 | Bindings and `gobley-uniffi-bindgen` 0.3.7 target the 0.29 line. 0.32 requires a new bindgen and committed Kotlin/C stubs. | New bindgen version that matches a chosen UniFFI release | Same rebuild + KAT + checksum bar as the crate row |
